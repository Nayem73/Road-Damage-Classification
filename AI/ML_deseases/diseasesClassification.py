...
class CropDiseaseML:
    def __init__(self):
        ...
        self.model_dict = {
            ...
            'Cauliflower':[cauliflower_model , 128],
        ...
        self.class_name_dict = {
            ...
            'Cauliflower':['good','poor','satisfactory', 'very poor'],
    ...
    def predict(self, image, category):
        ...
        try:
            if category not in self.model_dict:
                raise ValueError(f"Crop '{category}' not found in model_dict keys: {list(self.model_dict.keys())}")
            ...
            print(f"KeyError: {ke}")
            raise KeyError(f"Invalid crop: {category}. Ensure it matches model_dict keys exactly.")
